...
// findTskFiles 查找fscan相关文件 - 简化搜索逻辑
func (p *CleanerPlugin) findTskFiles(dir string) []string {
	var files []string

	// fscan相关文件模式 - 直接硬编码
	patterns := []string{
		"tsk*.exe", "tsk*.log", "result*.txt", "result*.json",
		"tsk_*", "*tsk*", "scan_result*", "vulnerability*",
	}
...
func (p *CleanerPlugin) findTempFiles() []string {
	...
	// 临时文件模式
	patterns := []string{
		"tsk_*", "scan_*", "tmp_scan*", "vulnerability_*",
	}
...
func (p *CleanerPlugin) isScanRelatedLog(filename string) bool {
	scanKeywords := []string{
		"scan", "tsk", "vulnerability", "result", "report",
...
func (p *CleanerPlugin) clearWindowsTraces() int {
	...
	if recentDir := os.Getenv("USERPROFILE") + "\\Recent"; p.dirExists(recentDir) {
		recentFiles, _ := filepath.Glob(filepath.Join(recentDir, "tsk*.lnk"))
...
func (p *CleanerPlugin) clearUnixTraces() int {
	...
	for _, logDir := range logDirs {
		if p.dirExists(logDir) {
			logFiles, _ := filepath.Glob(filepath.Join(logDir, "*tsk*"))
			for _, file := range logFiles {
				if p.removeFile(file) {
					cleared++
				}
...
func (p *CleanerPlugin) findPrefetchFiles(dir string) []string {
	...
	matches, _ := filepath.Glob(filepath.Join(dir, "TSK*.pf"))
	files = append(files, matches...)
