...
type UniversalPoc interface {
	...
	ToTskPoc() (*Poc, error) // 转换为fscan内部格式
}
...
// TskPocAdapter fscan原生格式适配器
type TskPocAdapter struct {
	*Poc
}

func loadTskPoc(data []byte) (*TskPocAdapter, error) {
	var poc Poc
	if err := yaml.Unmarshal(data, &poc); err != nil {
		return nil, fmt.Errorf("tsk格式解析失败: %w", err)
	}
	return &TskPocAdapter{&poc}, nil
}

// GetName 获取POC名称
func (f *TskPocAdapter) GetName() string {
	return f.Name
}

// GetFormat 获取POC格式类型
func (f *TskPocAdapter) GetFormat() PocFormat {
	return FormatFscan
}

// ToTskPoc 转换为Fscan POC格式
func (f *TskPocAdapter) ToTskPoc() (*Poc, error) {
	return f.Poc, nil
}
...
// ToTskPoc 将Nuclei格式转换为fscan格式
func (n *NucleiPocAdapter) ToTskPoc() (*Poc, error) {
	poc := &Poc{
		Name: n.GetName(),
		Detail: Detail{
...
// ToTskPoc 将xray格式转换为fscan格式
func (x *XrayPocAdapter) ToTskPoc() (*Poc, error) {
	poc := &Poc{
		Name:   x.Name,
		Detail: x.Detail,
	}
...
// ToTskPoc 将afrog格式转换为fscan格式
func (a *AfrogPocAdapter) ToTskPoc() (*Poc, error) {
